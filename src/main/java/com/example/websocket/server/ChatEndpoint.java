...
public class ChatEndpoint {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException, EncodeException {
        ...
        if (username.isEmpty()) {
            ...
            message.setContent("The user is invalid !");
    ...
    public void onMessage(Session session, Message message) throws IOException, EncodeException {
        Optional<Message> msg = Optional.ofNullable(message);
        if(msg.isPresent()){
            message.setFrom(users.get(session.getId()));
            if(message.getTo().size() > 0)
                sendMessageToSelectedUsers(msg.get(),session);
            else{
                message= getOnlineUsers(msg.get());
                broadcast(message);
            }
        }
    ...
    public void onError(Session session, Throwable throwable) {
        logger.error("session id : " + session.getId()  + throwable.getMessage());
    }
